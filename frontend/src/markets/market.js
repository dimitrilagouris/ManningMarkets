    ...
    return (
        <section className="market" aria-labelledby={market_id}>
            <header className="market__header">
                <h3 id={market_id} className="market__name">{name}</h3>
            </header>

            <div className="market__events" tabIndex="0" role="list" aria-label={`${market_id} events`}>
                {
                    events.map((ev) => {
                        const onYesClick = () => console.log(`Bet YES on ${ev.id}`);
                        const onNoClick = () => console.log(`Bet NO on ${ev.id}`);

                        return (
                            <div className="market__event" role="listitem" key={ev.id}>
                                <div className="event__info">
                                    <div className="event__name">{ev.name}</div>
                                    <div className="event__odds" aria-hidden="true">{ev.price}</div>
                                </div>
